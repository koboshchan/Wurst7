...
@SearchTags({"filler", "fill", "builder", "area fill", "air fill"})
public final class FillerHack extends Hack
	implements UpdateListener, RenderListener, GUIRenderListener
{
	...
	private final CheckboxSetting autoRestock =
		new CheckboxSetting("Auto restock",
			"Automatically enables Restock hack and pauses when out of blocks.",
			true);
	
	private final CheckboxSetting airPlace = new CheckboxSetting("Air place",
		"Places blocks in air like AirPlace hack, without requiring adjacent blocks.",
		false);
	...
	private FillerPathFinder pathFinder;
	...
	public String getRenderName()
	{
		...
		if(step == Step.FILL && area != null)
		{
			int totalBlocks = area.blocksList.size();
			double filledBlocks = totalBlocks - area.remainingBlocks;
			double progress = filledBlocks / totalBlocks;
	...
	protected void onEnable()
	{
		...
		// enable helpful hacks
		if(autoRestock.isChecked())
			WURST.getHax().restockHack.setEnabled(true);
	...
	public void onRender(MatrixStack matrixStack, float partialTicks)
	{
		...
		int gray = 0x26404040;
		int blue1 = 0x260080FF;
		int blue2 = 0x4D0080FF;
	...
	private void scanArea()
	{
		...
		// scan area for air blocks
		for(int i = 0; i < area.scanSpeed && area.iterator.hasNext(); i++)
		{
			area.scannedBlocks++;
			BlockPos pos = area.iterator.next();
			
			// Check if block is air and can be placed in
			if(isAirBlock(pos) && canPlaceAt(pos))
			{
	...
	private void fillArea()
	{
		...
		// Check if we're out of blocks (less than 5 remaining)
		if(autoRestock.isChecked() && stack.getCount() < 5)
		{
			currentBlock = null;
			// Let RestockHack handle restocking, we'll continue next tick
			return;
		}
		...
		if(MC.player.getAbilities().creativeMode && !legit)
		{
			...
			// place all blocks with packet spam
			for(BlockPos pos : validBlocks)
			{
				if(!placeBlockAt(pos))
					continue;
		...
		// get remaining air blocks
		area.remainingBlocks = (int)area.blocksList.parallelStream()
			.filter(this::isAirBlock).filter(this::canPlaceAt).count();
		...
		if(pathFinder == null)
		{
			BlockPos closestBlock = area.blocksList.parallelStream()
				.filter(this::isAirBlock).filter(this::canPlaceAt)
				.min(cNextTargetBlock).orElse(null);
			
			if(closestBlock != null)
				pathFinder = new FillerPathFinder(closestBlock);
		}
		...
		// check path
		if(processor != null && pathFinder != null
			&& !pathFinder.isPathStillValid(processor.getIndex()))
		{
			pathFinder = new FillerPathFinder(pathFinder);
			return;
		}
	...
	private boolean placeBlockAt(BlockPos pos)
	{
		...
		{
			// Use air placement like AirPlaceHack
			BlockHitResult hitResult = createAirPlaceHitResult(pos);
			if(hitResult == null)
				return false;
		...
		{
			// Use normal placement requiring adjacent blocks
	...
	private static class FillerPathFinder extends PathFinder
	{
		public FillerPathFinder(BlockPos goal)
		{
			super(goal);
			setThinkTime(10);
		}
		
		public FillerPathFinder(FillerPathFinder pathFinder)
		{
			super(pathFinder);
		}
